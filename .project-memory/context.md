...
<!-- date:2026-03-12 hash:d521003f7d59 -->
## Project Memory MCP

### Overview

The Project Memory MCP is a persistent AI knowledge base for Claude, a project management tool. It stores architectural decisions, technical debt, project progress, and domain knowledge in versioned Markdown files. The project uses the Model Context Protocol (MCP) to enable seamless interaction between the knowledge base and Claude.

### Features

#### MCP Resources

* 4 MCP Resources: `memory://decisions`, `memory://tech-debt`, `memory://progress`, `memory://context` are read automatically at session start.
* Additional MCP Resources: `get_memory`, `search_memory`, `reindex_memory`
...
* Embedding-based search via `@huggingface/transformers` (all-MiniLM-L6-v2) stored locally in SQLite

#### Git Hook

* Post-commit hook sends diff + message to Ollama for automatic summarization and auto-commits the updated memory files

#### Filesystem Watcher

* Changes to `CLAUDE.md`, `docs/adr/`, etc. trigger memory updates

#### Session Summary

* Inactivity timer writes a session summary to `progress.md`

#### Ollama Fallback

* Keyword extraction when Ollama is unavailable
...
This sets up everything automatically, including:
